"""
DNN-MLP baseline -- UNIFIED launcher (resubmission of AWPL-04-26-1519).

This script no longer owns its own data generation, SNR setting, decoding, or
loss. It imports everything from `unified_pipeline.py`, so the DNN-MLP baseline
...
network architecture (a deeper MLP). This directly removes the
...
for users who want to train/inspect the DNN-MLP baseline in isolation.

Usage:
    python DNN-MLP.py --aep_mat_path AEP_Matrix.mat --save_dir ./out_dnn_mlp
"""
import argparse
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Unified DNN-MLP baseline launcher")
    parser.add_argument("--aep_mat_path", type=str, default="AEP_Matrix.mat")
    parser.add_argument("--save_dir", type=str, default="./out_dnn_mlp")
    ...
    model = DNNMLP(input_dim=input_dim, num_elements=cfg.N, hidden=cfg.dnn_hidden, depth=cfg.dnn_depth, dropout=cfg.dnn_dropout)
    model, summary = train_standard_model(model, cfg, "dnn_mlp", input_dim, A_Etheta, A_Ephi, K, device, out_dir)
    ...
    pd.DataFrame([{ "best_epoch": summary["best_epoch"], "best_val_loss": summary["best_val_loss"],
                    ...
                    "EMA (%)": res["ema_pct"] }]).to_csv(out_dir / "summary_dnn_mlp.csv", index=False)
    print(f"DNN-MLP @ {cfg.nominal_eval_snr_db} dB: F1={res['f1_pct']:.2f}%, EMA={res['ema_pct']:.2f}%")
    print("Saved to:", out_dir / "best_checkpoint_dnn_mlp.pt")
